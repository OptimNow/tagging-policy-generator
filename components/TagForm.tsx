...
export const TagForm: React.FC<TagFormProps> = ({ tag, isRequired, onChange, onRemove, index }) => {
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-lg overflow-hidden mb-4 transition-all hover:border-white/20">
      {/* Header */}
      <div 
        className="flex items-center justify-between p-4 bg-white/5 cursor-pointer select-none"
            ...
            <>
              <div className="flex flex-col gap-1">
                  ...
                  {(tag as RequiredTag).validation_regex && (
                      <div className="mt-2 flex gap-2 items-center bg-black/20 p-2 rounded border border-white/5">
                          <span className="text-xs text-gray-400 uppercase tracking-wider">Test:</span>
                          <input 
                            className="bg-transparent border-none focus:ring-0 text-sm text-white flex-1"
